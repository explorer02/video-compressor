...
| 2026-08-01 | Library management: long-pressing a row enters selection mode, where tapping toggles rows, **All** selects everything in view and **Delete** removes them in one pass — reported honestly, including "3 of 5" when the OS dialog only partly went through. A **size filter** (any size, or ≥ 1 / 2 / 5 / 10 / 20 / 50 / 100 / 500 MB) hides everything smaller, for finding the videos actually worth acting on; it persists across launches and the header reads "12 of 240 videos" so a filtered list is never mistaken for a small library. Filtering shares the size sort's machinery — index once, then rank and page from memory — and is disabled on platforms with no size reader. This is batch _management_, not the batch compression §11 rules out.                                                                                                                                                                                                                        |
| 2026-08-01 | The selected video's screen now shows its created and modified times and the folder it lives in, read straight from the media store rather than by resolving a path. The preview gained an **Original / Compressed** switch above the player: one full-width player with the source swapped underneath, so the same scene can be flipped between encodes — which is what makes a quality difference actually visible, unlike two half-size players. The label-and-value rows both screens use are now one shared component.                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-08-01 | Compressed copies now save into the **source video's own folder** instead of the camera folder, and carry the original's capture and modified dates reliably. Both were the same root cause: the app created the asset and then wrote the dates, but a media store honours only the values a row is created with and may recompute or ignore the same columns on a later update — which is why the dates kept reporting as refused. Saving now happens through `media-tools`, which sets folder and both dates in the insert itself, keeps the row hidden until the bytes are written, and cleans up the placeholder if writing fails. Platforms without that capability keep the previous create-then-write-back path.                                                                                                                                                                                                                                                                |
| 2026-08-01 | Quality and player fixes. The HD tier now compresses in `manual` mode: in `auto` the library ignores the requested bitrate and applies its own envelope, which ceilings 720p at 2.0 Mbps and aims for ~1.5 Mbps for chat playback — so §5's 2.5 Mbps had never taken effect and the output was visibly soft. Its tagline changes from "WhatsApp HD" to "Great for sharing" to match. A tier below the source's resolution is now hidden rather than offered, since output is clamped to the source and "Full HD · 1080p" on a 720p video described a file the app could not produce. Video players regain the fullscreen control, which was explicitly disabled. Every finished compression also logs its speed against §7's ≥2× real-time target. Safety: an encoder that returns its input unchanged — which happens on some devices — is no longer adopted as output, because doing so moved the user's original into the temp directory and then deleted it for not being smaller. |
| 2026-08-01 | Added three browser features. Tapping a video now opens it in a real player with the platform's own play/pause/scrub controls instead of a still thumbnail. A **Delete** action removes a redundant video from the device without compressing it — it sits in the top bar rather than the action row so it stays reachable for a video that is already optimized, and, like Replace, it checks afterwards whether the OS dialog was actually confirmed. The preview now lists the original's created and modified times, folder, filename and resolution beside the size comparison — and its GPS coordinates where the platform supplies them — so the metadata a "keep original dates" save is promising is visible before choosing.                                                                                                                                                                                                                                                 |
| 2026-08-01 | Fix (first device run): stopping the compression notification always failed — the native call returned Android's `ComponentName`, which cannot cross the bridge — so the foreground service and its notification outlived every finished job. Date write-back now sets both columns in a single media-store update, accepts the modified date the provider recomputes rather than demanding an exact read-back, and reports precisely why a field was refused instead of a generic "rejected".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
